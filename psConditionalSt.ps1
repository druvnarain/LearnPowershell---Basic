...
if ("HELLO" -match "H") {
    #H exists in the string "HELLO"
    Write-Host "got a match"
}

# -notmatch     Results in true when a string doesn't match a regex pattern.
if ("HELLO" -notmatch "A") {
    #A does not match in the string "HELLO"
...
$list = @(1, 2, 3, 4, 5)
if (6 -notin $list) {
    Write-Host "6 is not in the list"
}
...
$var = "This is a string"
if ($var -isnot [bool]) {
    Write-Host "The variable is a string and not a Boolean value so results in true."
}
...
switch ($number) {
    ...
    default { Write-Host "I dont know what the number is" }
}
